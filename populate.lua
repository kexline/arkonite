...
local boxesLeft=24; -- TODO: not needed except in initial score bar
	...
	sceneGroup:insert(bg);
		...
		boxHit = function(event)
			...
			if event.phase=="ended" then
				...
				if (box.type=="red") then
					box.type="none"
					box:removeSelf();
					boxesLeft=boxesLeft-1;
					print("boxHit: red")
				elseif (box.type=="blue") then
					box.type="red"
					box:setFillColor(1,0,0);
					print("boxHit: blue")
				elseif (box.type=="yellow") then
					reverseColors();
					box:removeSelf();
					boxesLeft=boxesLeft-1;
					print("boxHit: yellow")
				-- no action on gray boxes
				end
		...
		populateBoxes = function(rows,cols)
			...
			for i=1,rows,1 do
				for j=1,cols,1 do
					...
					t=boxes[idx].type;
					if (t==1) then
						box:setFillColor(.5,.5,.5);	box.type="gray"
					elseif (t==2) then
						box:setFillColor(1,1,0);	box.type="yellow"
					elseif (t==3) then
						box:setFillColor(0,0,1);	box.type="blue"
					elseif (t==4) then
						box:setFillColor(1,0,0);	box.type="red";
					else
...
function scene:show( event )
	...
	elseif ( phase == "did" ) then
		...
		local function startGame(event)
			...
			print ("startGame")
			if cheat then
				statusText=display.newText("STILL CHEATING", xx, yy*1.2, gameFont, fs*1.25);

			else
				statusText=display.newText("Level 2", xx, yy*1.2, gameFont, fs*1.25);
			end
			statusText:setFillColor(.5,.5,.5,1);
			transition.fadeOut(statusText,{time=5000,onComplete={function() statusText:removeSelf() end}})

			ball:applyForce(5, 7, ball.x, ball.y);
		...
		local btnRestart=function(event)
			...
			composer.gotoScene("retry2", sceneOpt)	
		end
		...
		droppedBall = function(event)
			...
			if event.phase=="ended" then
				...
				if (cheat==false) then
					...
					btn=widget.newButton({
						onPress=btnRestart, id="restart", label="Try again", fontSize=fs, 
